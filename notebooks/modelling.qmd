...
Start simply, the goal being to just encourage thinking about what is meant by a marginal versus conditional view for odds-ratios on the treatment effect, irrespetive of whether there is common treatment parameter or not.

When I say marginal in relation to a causal treatment effect using an odds-ratio, I mean this, which follows the standard potential outcomes notation and definitions:
...
When I say conditional, I mean any situation where we might be thinking about strata specific effects
...
## Scenario 1 - how model specification influences parameter estimate in logistic regression
...
Assume (for now) that everyone in the cohort will be eligible and the options are soc vs soc + rifampin.
Include adjustment for joint (knee vs hip) with early acute silo having (abitrarily) more hip infections (early acute 60% chance of hip infection, late acute 35% chance of hip infection).
...
\begin{aligned}
y_i &\sim Bernoulli(\pi_i) \\
logit(\pi_i) &= \eta_i = \beta_0 + \beta_{s_{[i]}} + \beta_{abtype_{[i]}} + \beta_{joint_{[i]}}
\end{aligned}

where 

+ $\beta_0$ is a common intercept
+ $\beta_{s_{[i]}}$ models the effect of silo
+ $\beta_{abtype_{[i]}}$ models the effect of rifampin
+ $\beta_{joint_{[i]}}$ models the effect of joint (site of infection)

All terms are interpreted as *the change in the log-odds of treatment success under a unit change in the category type, holding all other terms constant*.

The effects are, by design, common across strata (urgh, silo). Assume:

+ $\beta_0 = logit^{-1}(0.55)$ (baseline response in early acute, soc <=60 is 55% treatment success)
+ $\beta_{\{s_{[i]} == earlyacute\}} = 0$
+ $\beta_{\{s_{[i]} == lateacute\}} = -0.5$ (worse outcomes in the late acute silo)
+ $\beta_{\{abtype_{[i]} == soc\}} = 0$
+ $\beta_{\{abtype_{[i]} == rif\}} = 0.6$ (rifampin works)
+ $\beta_{\{joint_{[i]} == knee\}} = 0$ 
+ $\beta_{\{joint_{[i]} == hip\}} = -0.7$ (hip decreases likelihood of treatment success)
...
Start with a very simple model and build up.
...
Fit a model with absolutely no stratification/adjustment, i.e. include only a term for assignment to rifampin.
...
The conditional model reflects the correctly specified model based on the data generating assumptions and accordingly the baseline risk aligns closer to what we know to be the truth: `r sprintf("%.2f", risk1)` (marginal) vs `r sprintf("%.2f", risk2)`  (conditional) models.

Similarly, exponentiating the two terms gives `r sprintf("%.2f", or_trt1)` and `r sprintf("%.2f", or_trt2)` for the estimated odds-ratios for the effect of rifampin under the marginal and conditional model.
...
There is an impact on the transportability of effects to other populations.
...
For the model specification, for now I am assuming the following:

\begin{aligned}
y_i &\sim Binomial(\pi_i, n_i) \\
logit(\pi_i) &= \eta_i = bsilo_{silo_{[i]}} + 
  bjoint_{joint_{[i]}} + \\
  &bsurg_{surg_{[i]}} +
  babdu_{abdu_{[i]}} +
  babty_{abty_{[i]}} 
...
which, I know, negates the possibility of subject level effects but that can be deemed irrelevant for now.
With priors:
...
with all bar $bsilo$ and $bjoint$ having sum-to-zero constraints applied and $bjoint_{[1]} = 0$.
Note that $bsilo$ is given a different sd simply to make it just about uniform on the probability scale and therefore allowing us to place a zero mean without major concern.
...
For the surgery domain, there were approx 700 patients in the early silo that would have received dair, but are put into against an early_na which will ensures they do not contribute to the estimation of dair effects.

You will note that the late acute has both one and two stage (instead of just revision).
...
I think this is in line with what SW raised but not entirely sure.
Using this approach is intended to accomodate different effects for one vs two stage in the late vs chronic, albeit that this is somewhat mute given that the surgery domain intends to aggregate one and two-stage in the late acute group into a revision category.
...
The presentation of these effects will be 
...
Similarly, for the antibiotic duration domain, there are approx 700 patients in the early silo that would have received long duration (12 weeks).
...
Of the approx 1250 in the late acute silo, approx 660 fell under either `dair` or `late_na`. 
...
Implement a model per the above spec:
...
The model produces estimates of risk by group, from which we could attempt to derive relevant causal effects (of whatever form required) by using some kind of empirical weighting.
