...
## Scenario 3 - model incorporating all strata and domains reparameterising randomisation effects
...
logit(\pi_i) &= \eta_i = bsilo_{[silo[i]]} + 
  bjoint_{[joint[i]]} + \\
  &bnotrandsurg_{[randsurg[i]]} + \\
  &I(randsurg[i] == Y) bsurg_{[surg[i]]} + \\
  &bnotrandabdu_{[randabdu[i]]} + \\
  &I(randabdu[i] == Y) babdu_{[abdu[i]]} + \\
  &bnotrandabty_{[randabty[i]]} + \\
  &I(randabty[i] == Y) babty_{[abty[i]]}
\end{aligned}

Make those randomised within a domain to be the reference group and not-randominsed an offset from this (common to all silos).

\begin{aligned}
bsilo_s &\sim N(0, 1.5) \\
bjoint_j &\sim N(0, 1) \\
bnotrandsurg &\sim N(0, 1) \\
bnotrandabdu &\sim N(0, 1) \\
bnotrandabty &\sim N(0, 1) \\
bsurg_o &\sim N(0, 1) \\
babdu_p &\sim N(0, 1) \\
babty_q &\sim N(0, 1) \\
\end{aligned}
...
$bsurg$, $babdu$ and $babty$ have sum-to-zero constraints over the randomised treatment options.
$bsilo_s$ is given a different sd to make it just about uniform on the probability scale and therefore allowing us to place a zero mean without major concern.
...
```{r}
#| label: tbl-demo3a
#| tbl-cap: "Demo data - summary for surgery domain"
#| tbl-cap-location: margin
 
dd3_smry <- dd3[, .(y = sum(y), .N), keyby = .(silo, joint, surg_rand, surg)]
dd3_smry[, pr_obs := y / N]
dd3_smry[, surg := factor(surg, levels = c(
  paste0("no_rand"),
  paste0("late_", c("dair", "one", "two")),
  paste0("chronic_", c("one", "two"))
  ))]

dd3_smry <- dd3_smry[order(silo, joint, surg_rand, surg)]
setkey(dd3_smry, silo, joint, surg_rand, surg)
...
```{r}
#| label: tbl-demo3b
#| tbl-cap: "Demo data - summary for antibiotic duration domain"
#| tbl-cap-location: margin
 
dd3_smry <- dd3[, .(y = sum(y), .N), keyby = .(silo, joint, abdu_rand, abdu)]
dd3_smry[, pr_obs := y / N]

dd3_smry[, abdu := factor(abdu, levels = c(
  paste0("no_rand"),
  paste0("late_one_", c("short", "long")),
  paste0("late_two_", c("short", "long")),
  paste0("chronic_one_", c("short", "long")),
  paste0("chronic_two_", c("short", "long"))
  ))]

dd3_smry <- dd3_smry[order(silo, joint, abdu_rand, abdu)]
setkey(dd3_smry, silo, joint, abdu_rand, abdu)
...
```{r}
#| label: tbl-demo3c
#| tbl-cap: "Demo data - summary for antibiotic type domain"
#| tbl-cap-location: margin
 
dd3_smry <- dd3[, .(y = sum(y), .N), keyby = .(silo, joint, abty_rand, abty)]
dd3_smry[, pr_obs := y / N]

dd3_smry[, abty := factor(abty, levels = c(
  "no_rand", "soc", "rif"
  ))]

dd3_smry <- dd3_smry[order(silo, joint, abty_rand, abty)]
setkey(dd3_smry, silo, joint, abty_rand, abty)
...
The following table provides a summary across each of the possible combinations of domain level classifications/assignments.
...
dd3_smry <- dd3[, .(y = sum(y), .N), keyby = .(silo, joint, surg_rand, surg, abdu_rand, abdu, abty_rand, abty)]
...
dd3_smry <- dd3_smry[order(silo, joint, surg, abdu, abty)]
setkey(dd3_smry, silo, joint, abty_rand, abty)
...
Revised model that includes parameters to characterise the effect of not being randomised and restricts the sum-to-zero constraint on the randomised treatments.

```{r}
#| class-output: stan
#| echo: false

cat(readLines(file.path("stan", "model04.stan")), sep = "\n")
```

Group the data to the set of covariate patterns then set indexes for stan.

:::callout-note
The use of negative indexes is just a workaround as stan doesn't like missing values in your data.
These negative indexes only appear under the domains (surg, abdu and abty) when no randomisation has occurred.
The model is set up such that these indexes are never actually encountered.
::: 

```{r}
dat <- dd3[, .(y = sum(y), n = .N), 
           keyby = .(silo, joint, surg_rand, surg, abdu_rand, abdu, abty_rand, abty)]
dat <- cbind(id = 1:nrow(dat), dat)

# X1
model.matrix(~silo + joint + surg_non_rand + abdu_non_rand + abty_non_rand,
             data = dat[, .(silo, joint, 
                            surg_non_rand = surg_rand == "N",
                            abdu_non_rand = abdu_rand == "N",
                            abty_non_rand = abty_rand == "N")])[,-1]

dat[silo == "early_acute" & abty_rand == "Y", .(id, abty = as.integer(abty == "rif"))]


dat[silo == "late_acute" & 
    surg_rand == "Y" & abdu_rand == "Y" & abty_rand == "Y", 
    .(id, abty = as.integer(abty == "rif"))]
...
model.matrix(~silo + joint + surg_non_rand + abdu_non_rand + abty_non_rand,
             data = dat[silo == "early_acute" & , .(silo, joint, 
                            surg_non_rand = surg_rand == "N",
                            abdu_non_rand = abdu_rand == "N",
                            abty_non_rand = abty_rand == "N")])[,-1]
...
id_surg <- 1:5
names(id_surg) <- c( 
  "late_dair", "late_one", "late_two",
  "chronic_one", "chronic_two"
)

id_abdu <- 1:8
names(id_abdu) <- c(
  "late_one_long", "late_one_short", "late_two_long", "late_two_short",
  "chronic_one_long", "chronic_one_short","chronic_two_long", "chronic_two_short"
)
...
dat[, id_silo := id_silo[silo]]
dat[, id_joint := id_joint[joint]]
dat[, id_randsurg := id_rand[surg_rand]]
dat[, id_randabdu := id_rand[abdu_rand]]
dat[, id_randabty := id_rand[abty_rand]]

dat[, id_surg := id_surg[surg]]
dat[, id_abdu := id_abdu[abdu]]
dat[, id_abty := id_abty[abty]]

dat[is.na(id_surg), id_surg := -999]
dat[is.na(id_abdu), id_abdu := -999]
dat[is.na(id_abty), id_abty := -999]
...
ld1 <- list(
  ...
  silo = dat$id_silo,
  joint = dat$id_joint,
  rand_surg = dat$id_randsurg,
  rand_abdu = dat$id_randabdu,
  rand_abty = dat$id_randabty,
  surg = dat$id_surg,
  abdu = dat$id_abdu,
  abty = dat$id_abty,
  prior_only = 0
...
m3 <- cmdstan_model(stan_file = file.path("stan", "model04.stan"))
f1 <- m3$sample(
  chains = 2,
  data = ld1,
  iter_sampling = 1000,
...
p1[, id := gsub("p[", "", par, fixed = T)]
p1[, id := as.integer(gsub("]", "", id, fixed = T))]
p1[, par := NULL]
...
```{r}
#| label: tbl-demo3-post2
#| tbl-cap: "Demo - parameter estimates"
#| tbl-cap-location: margin

p1 <- data.table(f1$draws(
  variables = c("b_silo", "b_joint[2]",
                "b_not_rand_surg[2]", "b_not_rand_abdu[2]", "b_not_rand_abty[2]",
                "b_surg", "b_abdu", "b_abty"), format = "matrix")) 
...
dtbl1[par %like% "b_silo", idx := gsub("b_silo[", "", par, fixed = T)]
dtbl1[par %like% "b_silo", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_silo", par_lev := names(id_silo[as.integer(idx)])]

dtbl1[par %like% "b_joint", idx := gsub("b_joint[", "", par, fixed = T)]
dtbl1[par %like% "b_joint", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_joint", par_lev := names(id_joint[as.integer(idx)])]

dtbl1[par %like% "b_not_rand_surg", idx := gsub("b_not_rand_surg[", "", par, fixed = T)]
dtbl1[par %like% "b_not_rand_surg", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_not_rand_surg", par_lev := names(id_rand[as.integer(idx)])]
dtbl1[par %like% "b_not_rand_abdu", idx := gsub("b_not_rand_abdu[", "", par, fixed = T)]
dtbl1[par %like% "b_not_rand_abdu", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_not_rand_abdu", par_lev := names(id_rand[as.integer(idx)])]
dtbl1[par %like% "b_not_rand_abty", idx := gsub("b_not_rand_abty[", "", par, fixed = T)]
dtbl1[par %like% "b_not_rand_abty", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_not_rand_abty", par_lev := names(id_rand[as.integer(idx)])]

dtbl1[par %like% "b_surg", idx := gsub("b_surg[", "", par, fixed = T)]
dtbl1[par %like% "b_surg", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_surg", par_lev := names(id_surg[as.integer(idx)])]

dtbl1[par %like% "b_abdu", idx := gsub("b_abdu[", "", par, fixed = T)]
dtbl1[par %like% "b_abdu", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_abdu", par_lev := names(id_abdu[as.integer(idx)])]

dtbl1[par %like% "b_abty", idx := gsub("b_abty[", "", par, fixed = T)]
dtbl1[par %like% "b_abty", idx := gsub("]", "", idx, fixed = T)]
dtbl1[par %like% "b_abty", par_lev := names(id_abty[as.integer(idx)])]

setcolorder(dtbl1, c("par", "par_lev"))
dtbl1[, idx := NULL]
...
gt_tbl <- dtbl1 |> 
  ...
  fmt_number(
    columns = all_of(c("mu", "sd", "q_025", "q_975")),
    decimals = 2
  )  |>
  opt_table_font(
    font = list(
...
There seems to be a flawed assumption with the sum-to-zero perspective across all treatments and silos.
It would be better to split these up.

How to do that in a sensible way?

Could use conditional logic.

Could I set up a standard design matrix for each combination?


```{r}
d <- data.table(grp = sample(1:3, 100, replace = T))
d[, grp := factor(grp)]

model.matrix(~-1 + grp, data = d)
```
...
# mtmp <- rstan::stan_model("stan//tmp.stan", auto_write = TRUE)
# rstan::expose_stan_functions(mtmp)
# m1 <- qr1(5)
# m2 <- qr2(5)
# 
# round(m1 %*% m2, 3)
