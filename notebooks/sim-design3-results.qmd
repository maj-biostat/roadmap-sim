...
# OR
g_or_lab <- c("OR 1/1.5" , "OR 1/1.2", "OR 1", "OR 1.2", "OR 1.5")
g_or_lev <- c("1/1.5", "1/1.2", "1", "1.2", "1.5")
g_or_num <- c(1/1.5, 1/1.2, 1, 1.2, 1.5)
names(g_or_lab) <- paste0("v0",c(1, 2, 3, 4, 5))
names(g_or_num) <- paste0("v0",c(1, 2, 3, 4, 5))

# log OR
g_lor_lab <- c("log-OR 1/1.5" , "log-OR 1/1.2", "log-OR 1", "log-OR 1.2", "log-OR 1.5")
names(g_lor_lab) <- paste0("v0", 1:5)
g_lor_num <- log(c(1/1.5, 1/1.2, 1, 1.2, 1.5))
...
2. For the duration domain (under both one-stage and two-stage revision) we consider whether short duration is non-inferior to long duration antibiotic treatment.
...
Specifically, we consider a non-inferiority margin of $\delta = \log(1/1.2)\approx -0.18$ and deem short course duration to be non-inferior to long course if $Pr(\beta_5 + \delta > 0) > 0.99$ for one-stage and $Pr(\beta_5 + \beta_6 + \delta > 0) > 0.99$ for two-stage.
We also consider futility to achieve non-inferiority whereby futility is indicated if the posterior probability of non-inferiority is less than 1%.
If either the short duration is non-inferior or futile, futher enrolment into this randomised comparison would be halted with the consequence that no more of the uncertainty will be resolved.

3. For the choice domain we evaluate whether rif is superior to no-rif.
We also consider futility.
...
d_fig <- rbind(
  ...
  d_fig[parname == "b_r2d" & quant %in% c("trt_ni_ref", "fut_trt_ni_ref"), ],
...
# analysis
d_tbl <- d_fig[quant %in% c("sup", "fut_sup", "trt_ni_ref", "fut_trt_ni_ref")]
...
g_tbl <- d_tbl |> gt(groupname_col = "parname") |> 
  ...
  tab_spanner(
    label = html("Futility (on ni)"),
    columns = c(paste0("fut_trt_ni_ref_", 1:5))
  ) |>
  cols_label(
    or_tru = html("OR (true)"),
    sup_1 = html("500"),
    ...
    fut_trt_ni_ref_1 = html("500"),
    fut_trt_ni_ref_2 = html("1000"),
    fut_trt_ni_ref_3 = html("1500"),
    fut_trt_ni_ref_4 = html("2000"),
    fut_trt_ni_ref_5 = html("2500")
  ) |>
  tab_style(
    style = cell_borders(
      sides = c("left"),
    ...
    locations = cells_body(
      columns = c(sup_1, fut_sup_1, trt_ni_ref_1, fut_trt_ni_ref_1)
      )
    ) |>
  tab_style(
    style = list(
