...
@tbl-sim-design3-par summarises the setup for the simulated ORs ranging from $1/2$ to $2$ on all arms. 
...
flist <- list.files("data", pattern = "sim03-sc02")

toks <- list()
l2 <- list()
for(i in 1:length(flist)){
  l2[[i]] <- qs::qread(file.path("data", flist[i]))
  toks[[i]] <-  unlist(tstrsplit(flist[i], "[-.]"))
}

d_cfg <- rbindlist(lapply(seq_along(l2), function(i){
  tok <- toks[[i]]
  m <- data.table(do.call(cbind, l2[[i]]$cfg))
  data.table(cbind(sc = tok[2], v = tok[3], analys = 1:nrow(m), m))
}))
...
i <- 1
d_dec_2 <- rbindlist(lapply(seq_along(l2), function(i){
  tok <- toks[[i]]

  d_decision <- copy(l2[[i]]$d_decision)
  m <- melt(d_decision, id.vars = c("sim", "analys", "quant"), variable.name = "parname")
...
@fig-pr-design3-1b shows the cumulative probability of each decision quantity with enrolment halted only for the questions detailed above when the futility threshold is increased to 5%.

```{r}
#| label: fig-pr-design3-1b
#| fig-cap: 'Probability of declaring decision by parameter by effect size (all pars set with same OR).'
#| fig-cap-location: margin
#| fig-height: 8
#| fig-width: 7
...
# put power by scenario, variant and analysis in long format
d_fig_0_05 <- melt(d_dec_2, id.vars = c("sc", "v", "analys", "parname"), variable.name = "quant")
d_fig_0_05[, quant := factor(quant, 
                        levels = c("sup", "fut_sup", "trt_ni_ref", "fut_trt_ni_ref"))]
# add in the number of pts having reached 12 months post rand by analysis num
d_fig_0_05 <- merge(d_fig_0_05, unique(d_cfg[, .(analys, N_pt)]), by = "analys")

d_fig_0_05 <- rbind(
  d_fig_0_05[parname == "b_r" & quant %in% c("sup", "fut_sup"), ],
  d_fig_0_05[parname == "b_r1d" & quant %in% c("trt_ni_ref", "fut_trt_ni_ref"), ],
  d_fig_0_05[parname == "b_r2d" & quant %in% c("sup", "fut_sup"), ],
  d_fig_0_05[parname == "b_f" & quant %in% c("sup", "fut_sup"), ]
)

d_fig_0_05[, or_tru := g_or_lab[v]]
d_fig_0_05[, or_tru := factor(
  or_tru, labels = g_or_lab, levels = g_or_lab)]

fx <- g_fx
names(fx) <- c("rev vs ref", "short vs ref (one)", "long vs ref (two)", "rif vs ref")
d_fig_0_05[, parname_lab := factor(
  names(fx[parname]), levels = c("rev vs ref", "short vs ref (one)", "long vs ref (two)", "rif vs ref")
  ) ]

d_text <- unique(d_fig_0_05[, .(parname_lab, or_tru, quant)])
d_text[parname_lab == "rev vs ref" & or_tru %in% c("OR 1/2"),
       `:=`(label = c("rev fut (sup)"), x = 500, y = 0.2)]
d_text[parname_lab == "rev vs ref" & or_tru %in% c("OR 2"),
       `:=`(label = c("rev sup \nto dair"), x = 500, y = 0.2)]
...
d_fig_0_05$quant <- factor(
  d_fig_0_05$quant, 
  levels = c("sup", "fut_sup", "trt_ni_ref", "fut_trt_ni_ref"),
  labels = c("sup", "fut_sup", "ni", "fut_ni"))

p <- ggplot(d_fig_0_05, aes(x = N_pt, y = value, group = quant, col = quant)) +
  geom_point(size = 0.5) +
  geom_line(lwd = 0.4) +
  geom_hline(yintercept = 0.05, lwd = 0.2) +
  ggthemes::scale_colour_tableau(
...
# Widen data so that power is shown by col with each col corresponding to an
# analysis
d_tbl <- d_fig_0_05[quant %in% c("sup", "fut_sup", "ni", "fut_ni")]
d_tbl <- dcast(d_tbl, parname + or_tru ~ quant + analys, value.var = "value")
d_tbl <- d_tbl[order(or_tru, parname)]
