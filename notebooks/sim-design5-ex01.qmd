...
# files of interest
flist <- list.files("data/sim05-06", pattern = "sim05")
toks <- list()
l <- list()
i <- 1
for(i in 1:length(flist)){
  l[[i]] <- qs::qread(file.path("data/sim05-06", flist[i]))
...
p_d2 <- ggplot(d_fig, aes(x = d2, y = cn)) +
  geom_linerange(aes(ymin = 0, ymax = cn), lwd = 1) +
  scale_x_discrete("") +
...
p_d3 <- ggplot(d_fig, aes(x = d3, y = cn)) +
  geom_linerange(aes(ymin = 0, ymax = cn), lwd = 1) +
  scale_x_discrete("") +
  scale_y_continuous("Sample size") +
  facet_grid(silo ~ analys, labeller = label_both) +
...
p_d4 <- ggplot(d_fig, aes(x = d4, y = cn)) +
  geom_linerange(aes(ymin = 0, ymax = cn), lwd = 1) +
  scale_x_discrete("") +
  scale_y_continuous("Sample size") +
  facet_grid(silo ~ analys, labeller = label_both) +
...
@fig-post-or and @fig-post-rd show the parameter estimates up until the simulated trial was stopped due to all questions having been answered.
...
For example, for domain 1 in analysis 1, the probability of revision being superior to DAIR (per the definition of superiority, which is a measure that is relative to nominated reference level for superiority) is approxiamtely 0.55. 
To make a superiority decision, this probability would have to exceed 0.98.

Similarly, for domain 1 in analysis 1, the probability of the superiority decision being futile (per the relevant definition) is approximately 0.3.
To make a futility decision (related to superiority) this probability has to fall *below* 0.25.
