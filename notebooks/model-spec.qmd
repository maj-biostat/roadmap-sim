...
$$
\begin{aligned}
\mathbb{E}[Y|L; \beta] &=  \text{expit}( \alpha_{[l,j]} + \\
...
+ $\alpha_{[l,j]}$ denotes the baseline log-odds of treatment success in silo $l$ and having infection site $j$
...
Independent standard normal priors are assumed for all treatment effects.
...
We define a non-inferiority margin^[To do - work out what this means on the risk scale and see if it makes any sense.] of $\phi = \log{1/1.2} \approx -0.18$ to be used across all treatment effects and consider $j$ to be non-inferior to $i$ if $Pr(\delta_{j,i} > \phi) > 0.99$.
