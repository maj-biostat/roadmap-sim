...
The surgical intervention will usually be dair for which the 12 weeks antibiotic duration is recommended.
There are instances where early stage infection patients will receive a surgical intervention.
These patients will be able to enter the duration domain for randomisation of long vs short dependent based on the type of revision received.
...
Both the planned surgery (one-stage/two-stage) should be captured at the time of randomisation and the surgical intervention actually performed should also be captured as this may deviate from the original plan and the duration domain is based on the surgery performed rather than the surgery that was originally planned.
...
These patients may be eligible to enter into the duration domain and so the duration domain effects will be informed by patients that entered into the surgical domain for randomised surgical treatment and also patients that did not receive randomised treatment.
...
This can lead to estimation problems that might necessitate model re-parameterisation and it is for this reason that the simulation model drops the $(1 - E_D)$ term:
...
| ID | Patient type |  Log-odds treatment success |
|-|----|----|
| 1 | Late silo receiving rand DAIR, non-rand 12 weeks AB and rand rif |  $\alpha_0 + \lambda_1 + \beta_{9}$  |
| 2 | Late silo receiving rand rev (one-stage), rand 12 weeks AB (ref group) and rand rif |  $\alpha_0 + \lambda_1 + \beta_4 + \beta_{9}$ |
| 3 | Late silo receiving non-rand rev (one-stage), rand 12 weeks AB (ref group) and rand rif | $\alpha_0 + \lambda_1 + \beta_2 + \beta_{9}$ |
| 4 | Chronic silo receiving non-rand rev (one-stage), rand 12 weeks AB (ref group) and rand rif | $\alpha_0 + \lambda_2 + \beta_2 + \beta_{9}$ |
| 5 | Late silo receiving non-rand rev (two-stage), rand 7 days AB (ref group) and rand rif | $\alpha_0 + \lambda_1 + \beta_3 + \beta_{9}$ |
| 6 | Chronic silo receiving non-rand rev (two-stage), rand 7 days AB (ref group) and rand rif | $\alpha_0 + \lambda_2 + \beta_3 + \beta_{9}$ |
| 7 | Late silo receiving rand rev (one-stage), rand 6 weeks AB and rand rif | $\alpha_0 + \lambda_1 + \beta_4 + \beta_6 + \beta_{9}$ |
| 8 | Late silo receiving non-rand rev (one-stage), rand 6 weeks AB and rand rif | $\alpha_0 + \lambda_1 + \beta_2 + \beta_6 + \beta_{9}$ |
| 9 | Chronic silo receiving non-rand rev (one-stage), rand 6 weeks AB and rand rif | $\alpha_0 + \lambda_2 + \beta_2 + \beta_6 + \beta_{9}$ |
| 10 | Late silo receiving non-rand rev (two-stage), rand 12 weeks AB and rand rif | $\alpha_0 + \lambda_1 + \beta_3 + \beta_7 + \beta_{9}$ |
| 11 | Chronic silo receiving non-rand rev (two-stage), rand 12 weeks AB and rand rif | $\alpha_0 + \lambda_2 + \beta_3 + \beta_7 + \beta_{9}$ |
...
The effect of interest is an average conditional log-odds ratio and is applicable to the late silo.
For the surgical domain, this amounts to a weighted combination of $\beta_2$ and $\beta_3$ where the weights are the sample based expectations for the probability of receiving one-stage and two-stage surgery.

$$
\begin{aligned}
\Delta_R = \beta_4 \mathbb{E}[S_{R_P} == 1] + \beta_5 \mathbb{E}[S_{R_P} == 2]
\end{aligned}
$$
