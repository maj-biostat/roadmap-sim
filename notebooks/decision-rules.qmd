...
Following the earlier model specification, let $\Delta_R = \beta_4 \mathbb{E}[\mathbb{I}(S_{R_P} == 1)] + \beta_5 \mathbb{E}[\mathbb{I}(S_{R_P} == 2)]$ correspond to the average conditional log-odds ratio associated with revision.
...
Let $\beta_8$ correspond to the conditional log-odds ratio associated with 12 weeks (long) duration antibiotics relative to 7 days (short) when two-stage revision is received.
