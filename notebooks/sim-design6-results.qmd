...
Here, we provide the results under a revised model where we allow for silo-specific effects in the surgical domain.
If, for whatever reason, the early or chronic silo show variation in the surgical domain effects then the silo adjustment is inadequate to address this and we end up with a poluted version of the surgical domain parameters.
...
The model isn't particularly complicated but the way that terms enter the model is convoluted and understanding the dependency implications and consequently interpretations is fairly challenging.
For the actual trial analysis, the model will be revised to the equivalent Bernoulli likelihood setup on unit level data and also extended to account for site, randomisation period and prognostic covariates.
...
for(i in 1:length(l)){
  ...
  d_1_tmp <- merge(
    d_1_tmp, 
    CJ(sim = 1:max(d_tru$sim), analys = 1:5, arm = 1:3, silo = 2, domain = 1), 
  ...
  d_2_tmp <- merge(
    d_2_tmp, 
    CJ(sim = 1:max(d_tru$sim), analys = 1:5, arm = 2:3, silo = 1:3, domain = 2), 
    by = c("sim", "analys", "arm", "silo", "domain"),
    all.y = T)
  ...
  d_3_tmp <- merge(
    d_3_tmp, 
    CJ(sim = 1:max(d_tru$sim), analys = 1:5, arm = 2:3, silo = 1:3, domain = 3), 
    by = c("sim", "analys", "arm", "silo", "domain"),
    all.y = T)
  ...
  d_4_tmp <- merge(
    d_4_tmp, 
    CJ(sim = 1:max(d_tru$sim), analys = 1:5, arm = 2:3, silo = 1:3, domain = 4), 
    by = c("sim", "analys", "arm", "silo", "domain"),
    all.y = T)
...
That is, when there is no true effect, we might still see effects as large as $\pm 0.2$ on the absolute risk scale.
...
@tbl-sim05-risk shows the empirical proportion having treatment success within the randomised comparisons across the simulations.
So, for example, the values for the surgical domain shows the empirical proportion having treatment success within the late acute silo by treatment arm (dair, rev(1), rev(2)).
