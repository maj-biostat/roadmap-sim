...
Patients may contribute to some or all domains, which influences the treatment regimen (combination of treatments across the domains) they receive and which suggests the various causal effects that might be identified.
...
generate_data_4 <- function(
    n = 1e6,
    g = function(er, ed, ef, ra, rp, d, f){ 
      -1 + 
        -0.1*(1-er) + (0.2*(ra == 1) + 0.1*(ra == 2))*er +
        -0.05*(1-ed) + (-0.07*ic + 0.2*d*(rp == 1) + 0.06*d*(rp == 2))*ed +
        -0.25*(1-ef) + 0.2*f*ef }
    ) {
  ...
  y <- rbinom(n, 1, plogis(g(er, ed, ef, ra, rp, d, f)))
...
fit2 <- glm(y ~ er:ra1 + er:ra2 +
              edx + ed:ic + ed:d:rp1 + ed:d:rp2 +
              efx + ef:f, data = D, family = binomial())
summary(fit2)
```

Adopt separate models.

```{r}
fit3 <- glm(y ~ r, data = D, family = binomial())
fit4 <- glm(y ~ d + rp2, data = D, subset = rp>0, family = binomial())
fit5 <- glm(y ~ f, data = D, family = binomial())
