...
$$
\begin{aligned}
\mathbb{E}[Y|R,X] &= \mathbb{E}[\mathbb{E}[Y|R,X,A,S]|R,X] \\
&= \sum_{s=0}^1\sum_{a=0}^2 
\text{expit}(\beta_0 + \beta_1\mathbb{I}(a=1) + \beta_2\mathbb{I}(a=2) + \beta_3s+\beta_4X)
\mathbb{P}(A=a|S=s,R,X)\mathbb{P}(S=s|R,X) \\
...
\text{expit}(\beta_0 + \beta_2 + \beta_3 + \beta_4X)\mathbb{P}(S=1|R=0,X) \\
...
w_s <- mean(D$s == 1)
w_s_x0_r0 <- mean(D[x==0&r==0]$s)
w_s_x1_r0 <- mean(D[x==1&r==0]$s)
w_s_x0_r1 <- mean(D[x==0&r==1]$s)
w_s_x1_r1 <- mean(D[x==1&r==1]$s)
...
EY_R0_2s_X0 <- expit(b2s[1]) * (1 - w_s_x0_r0) + expit(b2s[1] + b2s[4]) * w_s_x0_r0
EY_R0_2s_X1 <- expit(b2s[1] + b2s[5]) * (1 - w_s_x1_r0) + expit(b2s[1] + b2s[4] + b2s[5]) * w_s_x1_r0
EY_R1_2s_X0 <- expit(b2s[1] + b2s[2]) * (1 - w_s_x0_r1) + expit(b2s[1] + b2s[3] + b2s[4]) * w_s_x0_r1
EY_R1_2s_X1 <- expit(b2s[1] + b2s[2] + b2s[5]) * (1 - w_s_x1_r1) + expit(b2s[1] + b2s[3] + b2s[4] + b2s[5]) * w_s_x1_r1
EY_R0_2s <- w_x0 * EY_R0_2s_X0 + w_x1 * EY_R0_2s_X1
EY_R1_2s <- w_x0 * EY_R1_2s_X0 + w_x1 * EY_R1_2s_X1
...
rbind(
  ...
  "fit2s conditional (X = 0) log-odds ratio" = qlogis(EY_R1_2s_X0) - qlogis(EY_R0_2s_X0),
  "fit2s conditional (X = 1) log-odds ratio" = qlogis(EY_R1_2s_X1) - qlogis(EY_R0_2s_X1),
