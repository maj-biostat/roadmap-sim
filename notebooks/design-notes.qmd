...
This helps us formulate a picture of the joint probability distribution which allows us to simulate the data generation process.
Without the joint view, it is unclear how one would be able to simulate an internally consistent data set. 
It also allows us to determine which causal effects we can identify.
Even if we model all the domains separately, we still need a joint view of the distribution in order to simulate the data properly.

:::

Say we were doing a trial in just the late acute patients. We are interested in surgery (DAIR/revision) and antibiotic duration (long/short), but we are limited in that, for whatever reason, we can not ethically randomise the *type* of revision surgery, only revision surgery itself. 

However, choice of antibiotic duration is conditional on the type of revision surgery used, so surgery type needs to be considered in any joint analysis (alternatively, analyse separately).
...
However, this means that the reference group for the one-stage and two-stage duration effects have the same log-odds of treatment success, which is unlikely to be the case.
By splitting $R$ into one-stage and two-stage effects (see above) we allow the reference groups for the one-stage and two-stage duration effects to vary.
...
Ok, think I have convinced myself that it makes sense, if perhaps no-one else.
...
Still not quite sure what is being proposed below.
...
2. Patients may enter into the study without having being entered for a randomised comparison in the surgical domain (e.g. patients with early stage infection) in which case $R_A$ would have been determined entirely by clinician selection.
...
-   $E_R$ randomisation reveal for surgical domain - 0: no, 1: yes
-   $E_D$ randomisation reveal for duration domain - 0: no, 1: yes
-   $E_F$ randomisation reveal for choice domain - 0: no, 1: yes
-   $R$ randomised surgery - 0: DAIR, 1: revision
-   $S_R$ preferred surgery - 0: DAIR, 1: one-stage, 2: two-stage
-   $R_A$ allocated surgery - 0: DAIR, 1: one-stage, 2: two-stage
-   $R_P$ performed surgery (a noisy version of $R_A$?) - 0: DAIR, 1: one-stage, 2: two-stage
...
-   $D_F$ allocated choice - 0: norif, 1: rif, 2: other
...
For (a contrived) example, revision was randomised and the original surgeon only performs two-stage revision.
...
so that when we have no revealed randomisation ($E_R = 0$) for the surgical domain, $R_A$ aligns with the preferred procedure out of all those possible (dair, one-stage, two-stage).
However, when randomisation revealed for the surgical domain, the first term disappears and for $R = 0$, the allocation is dair, irrespective of preferred surgery, whereas when $R = 1$ we get the selected one-stage or two-stage procedure, which is the same approach as previously used.
...
when no reveal, $E_D = 0$ and $D_A = S_D$ (long/short/other duration) and when revealed, $D_A = D$ (long/short duration).
...
when no reveal, $E_F = 0$ and $F_A = S_F$ (norif/rif/other) and when revealed, $F_A = F$ (norif/rif).


::: {.callout-note collapse=false}

Above, I am assuming that you can randomise someone within a domain without first assessing their eligibility status, which I believe is the intention.
Randomisation would only be revealed if eligibility is confirmed, which is independent to the randomisation process.
...
Forget about silos for the moment.
The setting is such that a participant might be randomised in any/all of the surgical, duration and choice domains.
...
Consider randomised duration $D$; backdoor paths exists:

1. $D \leftarrow  R_P \leftarrow R_A \rightarrow Y$
2. $D \leftarrow  R_P \rightarrow E_D \rightarrow D_A \rightarrow Y$

The first is open, the second is closed due to the collider $D_A$. 
Minimally conditioning on the set $\{ {R_P}, {R_A} \}$ closes the backdoor path and allows identification of the total effect of duration on treatment success.

Consider choice $F$.
No backdoor paths exist and therefore no adjustment is required for the identification of the total causal effect of $F$ on $Y$.
...
\mathbb{E}[Y|L; \beta] &=  \text{expit}( \beta_0 + \\
  &\quad \beta_1 [1 - E_R] + [\beta_2 R + \beta_3 R \mathbb{I}(S_R = 2) ]E_R + \\
  &\quad \beta_4 [1 - E_D] + [\beta_5 I_{R_P} + \beta_6 D + \beta_{7} \mathbb{I}(R_P = 2) D]E_D + \\
  &\quad \beta_{8} [1 - E_F] + \beta_{9} F E_F  )
\end{aligned}
$$

where the $L$ denotes the model variables and $\beta$ the vector of parameters and the following describe the movements in the log-odds of treatment success associated with the non-reference levels:

+ $\beta_1$ is the shift under non-reveal (surgery^[The parameter ignores potential differentiation for surgical type.])
+ $\beta_2$ is the shift under revision with one-stage procedure
+ $\beta_3$ is the shift under revision with two-stage procedure
+ $\beta_4$ is the shift under non-reveal (duration) with no differentiation for surgical nor duration preference
+ $\beta_5$ is the shift for deviations from allocated revision to some an alternate surgical approach
+ $\beta_6$ is the shift for short duration when one-stage was actually performed
+ $\beta_7$ is the shift for short duration when two-stage was actually performed
+ $\beta_{8}$ is the shift under non-reveal (choice) with no differentiation for choice preference
+ $\beta_{9}$ is the shift for rif

Now, we have another complicating factor in that the surgical allocation may inform the type of surgery that the patient gets (but may deviate) and the randomisation that the patient is revealed to in the duration domain is determined by what surgical intervention the patient actually got.

Recalling the earlier rationale for splitting revision type, we now need to also adjust for when the planned and actual revision type deviate from one another.

::: {.callout-note collapse=false}

I am not clear on whether the above addresses the full impacts of deviations between allocated and performed surgery.
It simply aims to lump all deviations of any form from the planned surgery into a single parameter.
...
generate_data_4 <- function(
    n = 1e6,
    g = function(er, ed, ef, r, sr, ic, rp, d, f){ 
      -1 + 
        -0.1*(1-er) + (0.2*r + 0.4*r*sra)*er +
        -0.05*(1-ed) + (-0.07*ic + 0.9*d + 0.06*d*rp2)*ed +
        -0.25*(1-ef) + 0.15*f*ef 
      }
    ) {

  # all enter surgical for now
  er <- rep(1, n)
  # randomise dair vs rev
  r <- rbinom(n, 1, 0.5)
  
  # 70% chance of clin choosing two-stage should pt be rand to revision 
  # exchangeable across groups so can just create random values for whole sample
  sr <- numeric(n)
  ...
  # 30% of the allocated treatments get mixed up
  ic <- rbinom(n, 1, 0.3)
  rp <- ra
  rp[as.logical(ic)] <- sample(0:2, sum(ic), replace = T, prob = c(0.2, 0.2, 0.6))
  ed <- as.numeric(rp %in% 1:2)
  
  rp1 <- as.numeric(rp == 1)
  rp2 <- as.numeric(rp == 2)
  
  stopifnot(identical(rp>0, as.logical(ed)))
  
  # rand to long (0), short (1)
  ...
  y <- rbinom(n, 1, plogis(g(er, ed, ef, r, sra, ic, rp2, d, f)))
...
set.seed(1024)
D <- generate_data_4()
# D[, .N, keyby = .(d, ed, rp1, rp2, y)]

# erx isn't applicable because all are rand into surg
fit2 <- glm(y ~ er:r + er:r:sra +
              edx + ed:ic + ed:d + ed:d:rp2 +
              efx + ef:f, data = D, family = binomial())
summary(fit2)
...
For the marginal effect associated with surgery, we have 

$$
\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial R} = \beta_2 E_R + \beta_3 E_R \mathbb{I}(S_R = 2) 
$$

for the randomised comparisons, $E_R = 1$, then

$$
\mathbb{E}\left[\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial R}\right] = \beta_2 + \beta_3 \mathbb{E}[ \mathbb{I}(S_R = 2)]
$$

similarly, for duration, we have

$$
\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial D} = \beta_6 E_D + \beta_7 E_D \mathbb{I}(R_P = 2) 
$$

under one-stage, 

$$
\mathbb{E}\left[\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial D}\right] = \beta_6 \mathbb{E}[E_D]
$$

under two-stage, $R_P = 2$

$$
\mathbb{E}\left[\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial D}\right] = \beta_6 \mathbb{E}[E_D] + \beta_7 (Cov[E_D,\mathbb{I}(R_P = 2) ] + \mathbb{E}[E_D] \mathbb{E}[\mathbb{I}(R_P = 2) ] 
$$

and for choice

$$
\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial F} = \beta_9 E_F 
$$

since $E_F$ is fixed for the randomised comparison

$$
\mathbb{E}\left[\frac{\partial \mathbb{E}[Y|L;\beta]}{\partial F}\right] = \beta_9 
$$
...
dat <- data.table(dat)
dat <- dat[, .(Survived, PClass, SexCode, Age)][complete.cases(dat[, .(Survived, PClass, SexCode, Age)])]
mod <- glm(Survived ~ PClass * SexCode * Age, data = dat, family = binomial)
summary(mod)

nrow(model.matrix(mod))

nrow(dat)
d_out <- comparisons(mod,
  variables = list(PClass = c("1st", "3rd"))) # Step 1: Quantity

d_out <- data.table(d_out)
d_out
```



Empirically, these equate to

```{r, eval = T}
b <- coef(fit2) 
b_r_marg <- b["er:r"] * D[, mean(er)] + b["er:r:I(sr == 2)TRUE"] * D[, mean(er & sr == 2)]
b_d_1_marg <- b["ed:d"] * D[, mean(ed)]
b_d_2_marg <- b["ed:d"] * D[, mean(ed)] + b["ed:d:I(rp == 2)TRUE"] * cov(D[,ed],D[,as.numeric(rp == 2)]) + D[, mean(ed)]*D[, mean(rp == 2)]
b_f_marg <- b["ef:f"] * D[, mean(ef)] 
m <- data.table(
  estimate = c(
    "avg_condit_log_odds_ratio (surg)", 
    "avg_condit_log_odds_ratio (duration, one-stage)", 
    "avg_condit_log_odds_ratio (duration, two-stage)", 
    "avg_condit_log_odds_ratio (choice)"
  ),
  lor = c(
    b_r_marg, 
    b_d_1_marg,
...
# Short vs Long (one-stage) and short vs. long (two-stage)
avg_comparisons(fit2, variables = "d", by = "rp2", comparison = "lnoravg")
# comparisons(fit2, variables = list(d = c(0,1)), newdata = datagrid(rp2 = 0:1), comparison = "lnoravg")
