...
```{r}
#| label: generate-data-1-quantities
...
Now have two interventions: $R$ which has downstream unknown effects due to revision type assigned, and $D$ which is partially a consequence of revision type. 
...
so $\beta_1$ is the shift associated with one-stage long duration, $\beta_2$ the shift associated with two-stage long duration, $\beta_3$ the relative shift for short duration given one-stage, and $\beta_$ the relative shift for short duration given two-stage.
