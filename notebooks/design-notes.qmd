...
-   $S_{R_A}$ preferred surgery assuming randomised to revision - 0: one-stage, 1: two-stage
...
A revised view of the DAG is shown below, which clearly still has a number of simplications relative to the intended approach.
Patients may contribute to some or all domains, which influences the treatment protocol (combination of treatments across the domains) they receive and which lead us to the various causal effects that may be identified.
...
Forget all bar the late silo for the moment.
The setting is such that participants may be randomised in the surgical, duration and choice domains.
...
$$
\begin{aligned}
\mathbb{E}[Y|E_R, E_D, E_F, S_{R_A}, R, I_{R_P}, R_P, D, F] &= \text{expit}( \beta_0 + \\
  &\quad \beta_1 [1 - E_R] + [\beta_2 \mathbb{I}(R_A = 1) + \beta_3 \mathbb{I}(R_A = 2)]E_R + \\
  &\quad \beta_4 [1 - E_D] + [\beta_5 I_{R_P} + \beta_6 \mathbb{I}(R_P = 1) D + \beta_{7} \mathbb{I}(R_P = 2) D]E_D + \\
  &\quad \beta_{8} [1 - E_F] + \beta_{9} F E_F  )
\end{aligned}
$$

where the following denote movements in the log-odds of treatment success associated with the various covariate levels:

+ $\beta_1$ is the shift under non-reveal (surgery) with no differentiation for surgical preference
...
+ $\beta_5$ is the shift for deviations from allocated revision
...
When revealed for the surgical domain, exchangeability holds in the potential outcomes via $\mathbb{E}[Y(a)|A \in \{1,2\},E_R = 1] = \mathbb{E}[Y(a)|A = 0,E_R = 1] = \mathbb{E}[Y(a)|E_R = 1]$.
...
generate_data_4 <- function(
    n = 1e6,
    g = function(er, ed, ef, r, rp1, rp2, sra, d, f){ 
      -1 + 
        -0.1*(1-er) + (0.1*sra + 0.2*r + 0.1*r*sra)*er +
        -0.05*(1-ed) + (-0.07*ic + 0.2*d*rp1 + 0.06*d*rp2)*ed +
        -0.25*(1-ef) + 0.2*f*ef }
    ) {
  ...
  r <- rbinom(n, 1, 0.5)
  # unconstrained perspective on what surgical procedure should be done
  sr <- sample(0:2, n, replace = T, prob = c(0.2, 0.2, 0.6))
  # 70% chance of clin choosing two-stage once revision is required
  # exchangeable across groups so can just create random values for whole sample
  sra <- rbinom(n, 1, 0.7)
  ra <- (1 - er) * sr + er * r * (sra + 1) 
  ...
  y <- rbinom(n, 1, plogis(g(er, ed, ef, r, rp1, rp2, sra, d, f)))
...
fit2 <- glm(y ~ er:sra + er:r + er:sra:r +
              edx + ed:ic + ed:d:rp1 + ed:d:rp2 +
