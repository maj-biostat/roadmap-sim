...
$\beta_2$ is now the shift for revision, long duration and no-rif.
...
-   $S_{R_P}$ preferred surgery - 0: one-stage, 1: two-stage
...
2. when surgical domain is applicable (randomisation is revealed) then $S_R$ is the selection between one-stage and two-stage

In essence, it is a ranking of which surgical procedure is required.
...
Stuff on $D$ out of date. 
There is still an open path but now you need to adjust for ($R_P$ or $E_D$).
...
\mathbb{E}[Y|L; \beta] &=  \text{expit}( \beta_0 + \\
  ...
  &\quad \beta_4 [1 - E_D] + [\beta_5 \mathbb{I}(S_{R_P} \ne R_A) + \beta_6 R_P D + \beta_{7} R_P D \mathbb{I}(S_{R_P} = 2)]E_D + \\
...
generate_data_4 <- function(
    n = 1e6,
    g = function(er, ed, ef, r, sra, ic, rp, d, srp2, f){ 
      -1 + 
        -0.1*(1-er) + (0.2*r + 0.4*r*sra)*er +
        -0.05*(1-ed) + (-0.07*ic + 0.9*rp*d + 0.06*rp*d*srp2)*ed +
        -0.25*(1-ef) + 0.15*f*ef 
      }
    ) {
  ...
  ic <- rbinom(n, 1, 0.3)
  srp <- ra
  # the ones that swap-over revises the view on selection?
  srp[as.logical(ic)] <- sample(0:2, sum(ic), replace = T, prob = c(0.2, 0.2, 0.6))
  
  # still based on the rand allocation for dair/rev?
  rp <- as.numeric(srp %in% 1:2)
  
  # reveal entirely determined by rp therefore unnecessary???
  ed <- rp
  ...
  y <- rbinom(n, 1, plogis(g(er, ed, ef, r, sra, ic, rp, d, srp2, f)))
...
fit2 <- glm(y ~ er:r + er:r:sra +
              edx + ed:ic + ed:rp:d + ed:d:rp:srp2 +
