...
ix_trial <- l[[ix_scenario]]$cfg$ex_trial_ix[2]
...
For example, for domain 1 in analysis 1, the probability of revision being superior to DAIR (per the definition of superiority, which is a measure that is relative to nominated reference level for superiority) is approxiamtely `r sprintf("%.2f", d_fig_1[domain == 1 & ia == 1 & quant == "Superior", value])`. 
To make a superiority decision, this probability needs to exceed `r sprintf("%.3f", l_cfg$dec_probs$thresh_sup[1])`.

Similarly, for domain 1 in analysis 1, the probability of the superiority decision being futile (per the relevant definition) is approximately `r sprintf("%.2f", d_fig_1[domain == 1 & ia == 1 & quant == "Superior (futile)", value])`.
