...
Usually, in a multivariable model, our treatment effects represent the effect of intervening holding all other terms constant.
They isolate the effects of each factor within the factorial structure.
However, within this design, it is clear that we could report on *revision* effects that manifest solely as a function of effects under other domains.

This has nothing to do with the estimation process nor the statistical model, it is an artefact of the experimental design.
The problem is that we cannot decompose the variance into buckets that represent the effect of the surgical approach, effect of the extended prophylaxis etc.
In other words we are not able to isolate the effects of surgical intervention.

One possible way to respond to this is to define revision as a specific combination of strategies. The most accessible to a user of the information is probably going to be dair vs revision where dair corresponds strictly to dair plus 12 weeks backbone therapy and where revision conrresponds to one-stage revision with 12 weeks backbone antibiotic or two-stage revision with 12 weeks backbone antibiotic and no extended prophylaxis.
...
In no way does this fix the design, but it provides a more logical comparison than the mixture of strategies that is presently being thought of as representing the effect of surgical removal and replacement of a joint.
