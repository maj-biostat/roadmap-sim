...
Throughout, I will use the following notation:
...
The **total** effect of surgery ($A$) on outcome ($Y$) is clearly identifiable.
...
To get to a view on dair vs revision (the focus here) I use the g-formula to average over all the other terms and then use a weighted combination of the one-stage and two-stage effects based on the distribution of these in the sample.

The model is implemented in stan as follows:

```{stan output.var="ex1", code=readLines('stan/logistic-demo-03.stan')}
#| label: model-1
#| code-summary: Logistic regression with bootstrap
...
The stan model is fit repeatedly to simulated data sets each with 500 units.

```{r, eval = T, echo = T}
#| label: sim-1
#| code-summary: Simulation to examine long run properties of parameter estimate summaries
...
m_res <- do.call(rbind, mclapply(1:nsim, FUN = function(i){
  ...
  # f1 <- m1$sample(
  #     ld, iter_warmup = 500, iter_sampling = 1000,
  #     parallel_chains = 1, chains = 1, refresh = 0, show_exceptions = F, 
  #     max_treedepth = 10)
  
  snk <- capture.output(
      f1 <- m1$pathfinder(ld, num_paths=10, single_path_draws=40,
                             history_size=50, max_lbfgs_iters=100,
                             refresh = 0)
    )
  
  # f1$summary(variables = c("a0", "b"))
  
  d_post <- f1$draws(variables = c("a0","b1","b2","b3","b4"), format = "matrix")
  mu_params <- colMeans(d_post)
  names(mu_params) <- c("a0", names(g_pars))

  d_post <- f1$draws(variables = c("p"), format = "matrix")
  bias <- mean(colMeans(d_post) - d_s$p_y)
  
  d_post <- f1$draws(variables = c("marg_p0", "marg_p1", "marg_p2", "marg_p_rev"), format = "matrix")
  mu_p_marg <- colMeans(d_post)
  
  d_post <- f1$draws(variables = c("rd"), format = "matrix")
  mu_rd <- colMeans(d_post)
  
  c(mu_params, bias = bias, mu_p_marg, mu_rd, pr_rd_gt0 = mean(d_post[, 1] > 0))
    
}, mc.cores = 6))


d_smry <- data.table(m_res)
```

\newpage

The distribution of parameter means obviously does not align with the parameters used in the linear predictor as estimated parameters are on the log odds scale.
However, the magnitude of the effects should be in line with the parameters on the risk scale.

```{r, eval = T}
#| label: fig-par-means-1
#| code-summary: Distribution of posterior means (log-odds scale)
#| fig-cap: "Distribution of parameter means"
#| fig-cap-location: bottom
#| fig-height: 6
#| fig-width: 6
#| out-width: 80%
...
d_fig <- melt(d_smry[, cols, with = F], measure.vars = cols)
...
For each unit compute the expected value of $Y$ and subtract the expected value and then take the average over each dataset to estimate the bias.
Ideally this should be centered around zero.

```{r, eval = T}
#| label: fig-bias-1
#| code-summary: Expectation of bias over sample
#| fig-cap: "Expectation of bias over the simulated data set"
#| fig-cap-location: bottom
#| fig-height: 5
#| fig-width: 5
#| out-width: 70%
...
ggplot(d_fig, aes(x = value, group = variable)) +
  geom_density() +
  geom_vline(xintercept = mean(d_fig$value))
```
...
#| code-summary: Distribution of posterior means from marginal probability of outcome by surgical type
...
# marginal probs for surgical domain treatment dair, one, two, rev
...
Risk difference for dair vs rev.
Averaged over the simulated trials, the probability that the posterior risk difference (dair vs rev) was greater than zero was `r d_smry[, mean(pr_rd_gt0)]` (this value should be around 0.5 when the effect of revision is set to zero).

```{r, eval = T}
#| label: fig-rd-1
#| code-summary: Distribution of posterior means from risk difference (dair vs revision)
...
Equal magnitude revision effects are set for both one-stage and two-stage revision with worse outcomes for hip joints.
...
m_res <- do.call(rbind, mclapply(1:nsim, FUN = function(i){
  ...
  d_post <- f1$draws(variables = c("p"), format = "matrix")
  bias <- mean(colMeans(d_post) - d$p_y)
...
Estimated parameters are on the log odds scale.

```{r, eval = T}
#| label: fig-par-means-2
#| code-summary: Distribution of posterior means (log-odds scale)
#| fig-cap: "Distribution of parameter means"
#| fig-cap-location: bottom
#| fig-height: 6
...
Due to the fact that extended prophylaxis is only relevant for revision patients receiving a procedure, any extended prophylaxis effect will manifest as being an effect of revision.

> Is there any way around this in the setting where we are talking about generalised revision effects?
...
Parameter estimates on the log-odds scale.

```{r, eval = T}
#| label: fig-par-means-3
#| code-summary: Distribution of posterior means (log-odds scale)
#| fig-cap: "Distribution of parameter means"
#| fig-cap-location: bottom
#| fig-height: 6
