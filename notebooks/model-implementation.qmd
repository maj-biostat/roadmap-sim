...
```{r, eval = F}
m3 <- cmdstanr::cmdstan_model("stan/model-sim-03.stan")

set.seed(1)

sim_spec <- get_sim_spec()
sim_spec$gamma[, , "early"] <- 0
sim_spec$gamma[, , "late"] <- 0
sim_spec$gamma[, , "chronic"] <- 0
...
ll <- get_trial_data(N = 100, pop_spec = NULL, sim_spec = sim_spec)
ll$d
...
post <- data.table(f3$draws(variables = c(
    "alpha", 
    "b_a_l"
  ), format = "matrix"))
...
d_fig <- d_beta[, .(
  lor_med = median(value), 
  lor_q025 = quantile(value, prob = 0.025),
  lor_q975 = quantile(value, prob = 0.975)), keyby = variable]
```
