...
b_d <- do.call(rbind, mclapply(1:(min(nrow(eta_d_0), 1000)), function(ii){
  ...
  idx <- d_s[l1 == 1 & srp2 == 0, which = T]
  lo_0_l1_srp1 <- eta_d_0[ii, rep(idx, times = ld$n[idx])]
  lo_1_l1_srp1 <- eta_d_1[ii, rep(idx, times = ld$n[idx])]
  mu_l1_srp1 <- sum(lo_1_l1_srp1 - lo_0_l1_srp1) / sum(ld$n[idx])

  idx <- d_s[l1 == 1 & srp2 == 1, which = T]
  lo_0_l1_srp2 <- eta_d_0[ii, rep(idx, times = ld$n[idx])]
  lo_1_l1_srp2 <- eta_d_1[ii, rep(idx, times = ld$n[idx])]
  mu_l1_srp2 <- sum(lo_1_l1_srp2 - lo_0_l1_srp2) / sum(ld$n[idx])

  idx <- d_s[l2 == 1 & srp2 == 0, which = T]
  lo_0_l2_srp1 <- eta_d_0[ii, rep(idx, times = ld$n[idx])]
  lo_1_l2_srp1 <- eta_d_1[ii, rep(idx, times = ld$n[idx])]
  mu_l2_srp1 <- sum(lo_1_l2_srp1 - lo_0_l2_srp1) / sum(ld$n[idx])

  idx <- d_s[l2 == 1 & srp2 == 1, which = T]
  lo_0_l2_srp2 <- eta_d_0[ii, rep(idx, times = ld$n[idx])]
  lo_1_l2_srp2 <- eta_d_1[ii, rep(idx, times = ld$n[idx])]
  mu_l2_srp2 <- sum(lo_1_l2_srp2 - lo_0_l2_srp2) / sum(ld$n[idx])
  
  c("mu" = mean(lo_1 - lo_0), 
    # you need to base the mean on the n that were used in the strata
    "mu_l1_srp1" = mu_l1_srp1,  "mu_l1_srp2" = mu_l1_srp2,
    "mu_l2_srp1" = mu_l2_srp1, "mu_l2_srp2" = mu_l2_srp2
    )
...
Results and comparison to frequentist model:
...
avg_comparisons(f2, variables = "d", comparison = "lnor", by = c("l2", "l1", "srp2"))
avg_comparisons(f2, variables = "d", comparison = "lnor", by = c("l2", "l1", "srp2"))
colMeans(b_d)
...
Alternative approach e.g. for `r`:
