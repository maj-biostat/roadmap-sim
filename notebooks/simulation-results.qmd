...
Summarises superiority for fixed trial when all domains treatment effects are set to the same values ranging from $\log(1/1.4)$ to $\log(2)$ and with one-side decision mechanisms using fixed thresholds, see @tbl-sim-par.
...
d_sup <- rbindlist(lapply(l, function(z){
  cbind(sc = z$cfg$sc, z$d_sup[, lapply(.SD, mean)])
}))

# Column `b_b1_c_2` doesn't exist.
d_sup <- melt(d_sup, id.vars = "sc", value.name = "pr_win")
...
d_sup <- merge(d_sup, d_pars, by = c("sc", "variable"))  
d_sup[, lor_tru := as.numeric(lor_tru)]
parname_map <- roadmap.data::get_par_effects_mapping()
d_sup[, parname := parname_map[variable]]
d_sup[, parname := factor(parname, levels = roadmap.data::get_par_effects_mapping())]
```
...
@fig-pr-win-1 summarises the change in the probability of declaring superiority on each parameter as the effects sizes vary.
...
ggplot(d_sup, aes(x = exp(lor_tru), y = pr_win)) +
  geom_point(size = 0.5) +
  geom_line() +
...
## One effective treatment, one harmful
