...
data{ 
  ...
  array[N1+N2] int d2_1;
  // array[N1+N2] int d3_1;
  ...
  vector[ncXd1] sd1;
  // domain 2 antibiotic backbone duration
  int nrXd2;
  int ncXd2;
  matrix[nrXd2, ncXd2] Xd2des;
  vector[ncXd2] sd2;
  // domain 3 extended prophylaxis
  // int nrXd3;
  // int ncXd3;
  // matrix[nrXd3, ncXd3] Xd3des;
  // vector[ncXd3] sd3;
...
transformed data{
  ...
  matrix[N1, ncXd2] X1d2 = Xd2des[d2_1[ixs1]];
  // matrix[N1, ncXd3] X1d3 = Xd3des[d3_1[ixs1]];
  ...
  matrix[N2, ncXd2] X2d2 = Xd2des[d2_1[ixs2]];
  // matrix[N2, ncXd3] X2d3 = Xd3des[d3_1[ixs2]];
  ...
  matrix[N3, ncXd2] X3d2 = Xd2des[d2_2[ixs3]];
  // matrix[N3, ncXd3] X3d3 = Xd3des[d3_2[ixs3]];
...
transformed parameters{
  vector[N1] eta1 = mu + X1s*bs + X1j*bj + X1p*bp + X1g1*bg1 + X1d2*bd2;
  vector[N2] eta2 = mu + X2s*bs + X2j*bj + X2p*bp + X2g1*bg1 + X2d2*bd2;
  vector[N3] eta3 = mu + X3s*bs + X3j*bj + X3p*bp + X3d1*bd1 + X3d2*bd2;
  vector[N] eta;
  eta[1:N1] = eta1;
  eta[(N1+1):(N1+N2)] = eta2;
  eta[(N1+N2+1):N] = eta3;
