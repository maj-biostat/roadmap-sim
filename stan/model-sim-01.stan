...
model{
  ...
  // early
  target += bernoulli_logit_lpmf(e_y | alpha[e_su] + e_ec * gamma_c + e_ecp .* b_c[e_c]) ; 
  // late
  target += bernoulli_logit_lpmf(l_y | alpha[l_su] + l_eb * gamma_b + l_ec * gamma_c + 
                                  l_eap .* b_a_l[l_a] +
                                  l_eb1p .* b_b1_l[l_b] + l_eb2p .* b_b1_l[l_b] +
                                  l_ecp .* b_c[l_c]) ; 
  // chronic
  target += bernoulli_logit_lpmf(c_y | alpha[c_su] + c_eb * gamma_b + c_ec * gamma_c + 
                                  c_eap .* b_a_c[c_a] +
