...
if (length(args)<1) {
  ...
  args[1] = "run_none_sim_06"
  args[2] = "./sim06/cfg-sim06-sc01-v01.yml"
} else {
...
model_consistency_check <- function(){
  ...
  names(d_r) <- paste(rep(c("m1", "m2"), each = ncol(d_r)/2), names(d_r), sep = "_")
  colMeans(d_r)
  ...
  d_tbl[, variable := gsub("m2_", "", variable, fixed = T)]
  d_tbl <- dcast(
...
run_none_sim_06 <- function(){
  log_info("run_none_sim_06: Nothing doing here bud.")
}

main_sim_06 <- function(){
